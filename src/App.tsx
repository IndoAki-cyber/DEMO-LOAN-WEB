  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#F3F6FA] text-[#102b3a]">
      ...
      <main>
        ...
        {/* PROCESS */}
        <section id="process" className="relative overflow-hidden bg-gradient-to-br from-[#F3F6FA] via-[#DDF5F4] to-[#F3F6FA] px-5 py-24 md:px-8 md:py-32"><div className="float-orb absolute -left-20 top-40 h-80 w-80 rounded-full bg-[#DDF5F4] blur-3xl opacity-50" /><div className="dot-float absolute right-12 top-20 h-3 w-3 rounded-full bg-[#08B3B5] shadow-[0_0_12px_rgba(8,179,181,.5)]" /><div className="dot-float-slow absolute right-1/4 bottom-16 h-2.5 w-2.5 rounded-full bg-[#08B3B5] shadow-[0_0_12px_rgba(8,179,181,.5)]" /><div className="relative mx-auto max-w-7xl"><Reveal variant="up"><div className="max-w-xl"><p className="eyebrow">A clearer way forward</p><h2 className="section-title">How it works</h2><p className="mt-5 text-base leading-7 text-[#5C748A]">From your first question to your next step, the journey stays simple and easy to follow.</p></div></Reveal><div className="relative mt-16 grid gap-10 md:grid-cols-4 md:gap-5">{[['01', 'Tell us what you need', 'Begin with your financial requirement.'], ['02', 'Share your details', 'Give us the information needed to understand your enquiry.'], ['03', 'Explore suitable options', 'Review the possibilities relevant to your situation.'], ['04', 'Proceed with next steps', 'Move ahead with clarity and confidence.']].map(([number, title, text], index) => <Reveal key={number} delay={index * 140} variant="up"><div className="group relative flex gap-5 md:block"><div className="relative z-10 flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border-4 border-[#F3F6FA] bg-gradient-to-br from-[#08B3B5] to-[#082B52] font-display text-base font-bold text-white shadow-[0_8px_20px_rgba(8,179,181,0.25)] transition duration-500 group-hover:scale-110 group-hover:shadow-[0_12px_30px_rgba(8,179,181,0.35)] md:h-16 md:w-16 md:text-lg">{number}</div><div className="md:mt-8"><h3 className="text-lg font-bold capitalize text-[#082B52]">{title}</h3><p className="mt-2 max-w-[210px] text-sm leading-6 text-[#5C748A]">{text}</p></div>{index < 3 && <div className="absolute left-7 top-14 h-[calc(100%+40px)] w-0.5 bg-gradient-to-b from-[#08B3B5]/40 to-[#08B3B5]/10 md:left-8 md:top-16 md:h-0.5 md:w-[calc(100%+20px)] md:bg-gradient-to-r" />}</div></Reveal>)}</div></div></section>
