      ...
      <main>
        ...
        <section id="process" className="relative overflow-hidden bg-white px-5 py-24 md:px-8 md:py-32">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[.92fr_1.08fr] lg:gap-20">
            <Reveal variant="left">
              <div>
                <p className="eyebrow text-[#C92E54]">Multi-lender access</p>
                <h2 className="mt-5 max-w-xl font-display text-5xl font-extrabold leading-[1.06] tracking-[-0.055em] text-[#082B52] sm:text-6xl">One Application.<br />50+ Lenders.<br />Best Rate <span className="text-[#C92E54]">Guaranteed.</span></h2>
                ...
                <button onClick={() => scrollTo('contact')} className="group mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#C92E54] transition hover:gap-3">Explore our lender network <ArrowRight size={16} className="transition group-hover:translate-x-1" /></button>
                <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {[[Building2, '50+ Lenders', 'In One Place'], [TrendingUp, 'Best Rate', 'Guaranteed'], [Zap, 'Saves Time', '& Effort'], [ShieldCheck, '100% Secure', '& Trusted']].map(([Icon, title, text], index) => {
                    ...
                    return <Reveal key={title as string} delay={index * 100} variant="tilt"><div className="group rounded-2xl border border-[#E7EBF0] bg-[#FAFBFC] p-4 text-center transition duration-300 hover:-translate-y-1 hover:border-[#F1B5C2] hover:bg-white hover:shadow-[0_14px_30px_rgba(201,46,84,0.1)]"><div className="mx-auto flex h-9 w-9 items-center justify-center rounded-full bg-[#FFF0F3] text-[#C92E54] transition group-hover:scale-110"><BenefitIcon size={17} /></div><p className="mt-3 text-[11px] font-extrabold text-[#082B52]">{title as string}</p><p className="text-[10px] leading-4 text-[#9AA8B5]">{text as string}</p></div></Reveal>;
                  })}
                </div>
              </div>
            </Reveal>
            <Reveal variant="right" delay={140}>
              <div className="group relative overflow-hidden rounded-[28px] border border-[#E7EBF0] bg-white p-2 shadow-[0_24px_70px_rgba(8,43,82,0.13)] transition duration-500 hover:-translate-y-2 hover:shadow-[0_32px_90px_rgba(8,43,82,0.18)]">
                ...
                <div className="pointer-events-none absolute bottom-6 left-6 rounded-full border border-white/80 bg-white/90 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[#C92E54] shadow-lg backdrop-blur-sm opacity-0 transition duration-500 group-hover:opacity-100">One trusted network</div>
              </div>
            </Reveal>
          </div>
        </section>
